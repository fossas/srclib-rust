...
fn parse_cargo_meta(args: &ParsedArgs) -> Result<Metadata> {
    MetadataCommand::new()
    .manifest_path(get_manifest_path(&args.subdir))
    .exec().map_err(|e| e.into())
}
...
fn get_this_package(meta: &Metadata) -> &Package {
    let source_id = meta.workspace_members.first().expect("No workspace members found, cannot determine the correct package.");
    meta.packages.iter().find(|p| p.id == *source_id).expect("No known package matched the workspace member.")
}

fn get_direct_deps(pkg: &Package, args: &ParsedArgs) -> Result<Vec<ResolvedDependency>> {
    pkg.dependencies.iter().cloned().map(|dep| try_dep_xform(dep, args)).collect()
}
...
fn try_dep_xform(dep: Dependency, args: &ParsedArgs) -> Result<ResolvedDependency> {
    ...
    .source(dep.source.unwrap_or_else(|| "registry+https://github.com/rust-lang/crates.io-index".into()))
...
fn get_source_files<P: AsRef<Path>>(root: P) -> Result<Vec<PathBuf>> {
    ...
    .map(|r| r.map_err(|e| e.into())).collect()
}
